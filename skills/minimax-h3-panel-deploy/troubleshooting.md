...
## 容器里 ComfyUI 起不来：`comfy/ldm/models` 相关 SyntaxError

- **ComfyUI 0.32.0 的 `comfy/ldm/models/` 目录只有 `autoencoder.py`，没有 `__init__.py`**（PEP 420 命名空间包不需要它）。GitHub raw 拉 `__init__.py` 返回 404 是正常现象；**把 404 文本存成文件放进去反而炸出 `SyntaxError: illegal target for annotation`**（2026-09-20 690 实测）。正确修法=只补 `autoencoder.py`（与官方 release 逐字节一致，v0.32.0 md5 `41ade1c8`）+ 删假 `__init__.py`。
- 报缺 `comfy/ldm/models/autoencoder.py`：多数是构建/同步时 `--exclude models` 把树里**所有**名为 models 的目录都排除了（含 Python 包）。rsync 排除顶层模型目录要写成锚定的 `--exclude /models`。

## 单容器镜像（deploy/docker/）启动崩溃或驱动问题

- 容器入口**禁止**把整个宿主机 `/usr/lib/x86_64-linux-gnu` 塞进 `LD_LIBRARY_PATH`（宿主 glibc 与容器不匹配 → `/entrypoint.sh: line N: Aborted (core dumped)` stack smashing）。entrypoint 的做法是只挑出 NVIDIA 相关库拷到 `/driver-libs/` 再注入。
- 镜像里缺 `/app/ComfyUI/models` 挂载点：entrypoint 在 symlink 前有 `mkdir -p /app/ComfyUI/models`，别删；构建上下文 rsync 同样注意上一条的 exclude 锚定。
- UGREEN NAS（UGOS Pro）`docker load` 后镜像内可能静默丢文件（containerd 存储怪癖）：先 `docker run --rm --entrypoint ls <tag> /app/ComfyUI/comfy/ldm/models` 验证；确认丢文件就 `docker cp` 补进在跑容器再 restart（临时），根治=换完整 tar 重载。
- UPK/商店 compose 只发布 8189：宿主机 `curl 127.0.0.1:8188` 返回 000 是正常现象，ComfyUI 存活要 `docker exec` 进容器 curl。
- 面板 API 的已装模型清单在 `/api/models` 顶层 `installed` 数组；`available` 数组里的 `installed` 字段不反映实际（写脚本别读错）。
