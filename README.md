...
**通宵批量（v6.0）**
- 贴一批提示词（每行一条）× 每条 1/2/4 个种子，一次挂机最多 240 个任务，全自动逐个生成
- 首帧图批量图生视频（v7.0）：每行可配一张首帧图，第 i 张对应第 i 行
- 跑完自动拼接成合集（v7.0）：全部成功片段无损拼为一条长片入库
- 关浏览器不影响；面板重启自动续跑剩余队列（含在途任务收尾）
- 单个失败自动跳过并记录原因；ComfyUI 断连按分钟级自动重试
- 可选每片自动插帧/超分；进度条 + 预计完成时刻 + 完成提醒 + 失败汇总
- 与创作台共用队列：通宵跑着也可以随时插队临时任务

**容量管理（v7.0）**
- 作品库按分类（生成/插帧/超分/拼接）统计数量与体积、星标占用、磁盘剩余
- 两步式清理：预览待删数量与体积 → 确认执行；固定保护星标与最近 7 天作品
